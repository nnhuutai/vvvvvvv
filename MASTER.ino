...
#define BLYNK_DEVICE_NAME "ỨNG DỤNG MẠNG TRUYỀN THÔNG LORA TRONG NUÔI TÔM"
...
BLYNK_WRITE(V7) {
  ...
  if (virtualStatus == 0) {
    ...
    e32ttl.sendFixedMessage(0, 3, 0x04, "TATLAMP");
  }
}
BLYNK_WRITE(V8) {
  int virtualStatus = param.asInt(); 
...
BLYNK_WRITE(V12) {
  ...
  if (virtualStatus == 0) {
    ...
    e32ttl.sendFixedMessage(0, 3, 0x04, "TATCLEAN");
  }
}
BLYNK_WRITE(V10) {
  int virtualStatus = param.asInt(); 
  if (virtualStatus == 0) {
    ...
    e32ttl.sendFixedMessage(0, 3, 0x04, "TATDRAIN");
  }
}
...
BLYNK_WRITE(V11) {
  ...
  if (virtualStatus == 0) {
    ...
    e32ttl.sendFixedMessage(0, 3, 0x04, "TATFEED");
  }
}
...
void updateDataToBlynk() {
  ...
  //Đẩy dữ liệu lên các Virtual tương ứng
...
void updateButton() {
  if (Data_From_Lora.indexOf("BATLAMP") >=0 ) {
    TT_LAMP= LOW;
    Blynk.virtualWrite(V7, 1);
  }else if (Data_From_Lora.indexOf("TATLAMP") >=0 ) {
    TT_LAMP = HIGH;
    Blynk.virtualWrite(V7, 0);
  }
  
  if (Data_From_Lora.indexOf("BATPUMP") >=0 ) {
    statusEC=1;
    TT_PUMP= LOW;
    Blynk.virtualWrite(V8, 1);
  }else if (Data_From_Lora.indexOf("TATPUMP") >=0 ) {
    TT_PUMP = HIGH;
    Blynk.virtualWrite(V8, 0);
    statusEC=0;
  }

  if (Data_From_Lora.indexOf("BATFAN") >=0 ) {
    TT_FAN= LOW;
    Blynk.virtualWrite(V9, 1);
  }else if (Data_From_Lora.indexOf("TATFAN") >=0 ) {
    TT_FAN = HIGH;
    Blynk.virtualWrite(V9, 0);
  }
 
  if (Data_From_Lora.indexOf("BATCLEAN") >=0 ) {
    TT_CLEAN= LOW;
    Blynk.virtualWrite(V12, 1);
  }else if (Data_From_Lora.indexOf("TATCLEAN") >=0 ) {
    TT_CLEAN = HIGH;
    Blynk.virtualWrite(V12, 0);
  }
    if (Data_From_Lora.indexOf("BATDRAIN") >=0 ) {
    TT_DRAIN= LOW;
    Blynk.virtualWrite(V10, 1);
  }else if (Data_From_Lora.indexOf("TATDRAIN") >=0 ) {
    TT_DRAIN = HIGH;
    Blynk.virtualWrite(V10, 0);
  } 
      if (Data_From_Lora.indexOf("BATFEED") >=0 ) {
    TT_FEED= LOW;
    Blynk.virtualWrite(V11, 1);
  }else if (Data_From_Lora.indexOf("TATFEED") >=0 ) {
    TT_FEED = HIGH;
...
void handleTimerID1(){
  ...
  if(runMode==1){
    if(floatEC>mucCanhbao){
      if(canhbaoState==0){
        canhbaoState=1;
        Blynk.logEvent("canhbao", String("Cảnh báo! nồng độ EC = " + String(floatEC)+" vượt quá ngưỡng cho phép!"));
    ...
    if(floatDO>mucCanhbao){
      if(canhbaoState==0){
        canhbaoState=1;
        Blynk.logEvent("canhbao", String("Cảnh báo! nồng độ DO = " + String(floatDO)+" vượt quá ngưỡng cho phép!"));
        Blynk.virtualWrite(TRANGTHAICB,HIGH);
        callNumber();
        delay(100);
        SendMessageDO();
    ...
    if(floatTDS>mucCanhbao){
      if(canhbaoState==0){
        canhbaoState=1;
        Blynk.logEvent("canhbao", String("Cảnh báo! nồng độ TDS = " + String(floatTDS)+" vượt quá ngưỡng cho phép!"));
        Blynk.virtualWrite(TRANGTHAICB,HIGH);
        callNumber();
        delay(100);
        SendMessageTDS();
    ...
    if(floatTSS>mucCanhbao){
      if(canhbaoState==0){
        canhbaoState=1;
        Blynk.logEvent("canhbao", String("Cảnh báo! nồng độ TDS = " + String(floatTSS)+" vượt quá ngưỡng cho phép!"));
        Blynk.virtualWrite(TRANGTHAICB,HIGH);
        callNumber();
        delay(100);
        SendMessageTSS();
